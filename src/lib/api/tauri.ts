...
export async function getDiff(repoPath: string, revision: string): Promise<DiffResult> {
  return invoke<DiffResult>('get_diff', { repoPath, revision });
}

export async function getConflicts(repoPath: string, revision: string): Promise<ConflictFile[]> {
  return invoke<ConflictFile[]>('get_conflicts', { repoPath, revision });
}

export async function saveComment(repoPath: string, comment: Comment): Promise<Comment> {
  return invoke<Comment>('save_comment', { repoPath, comment });
}

export async function getComments(repoPath: string, revision?: string): Promise<Comment[]> {
  return invoke<Comment[]>('get_comments', { repoPath, revision });
}

export async function deleteComment(repoPath: string, commentId: string): Promise<void> {
  return invoke<void>('delete_comment', { repoPath, commentId });
}

export async function resolveComment(repoPath: string, commentId: string): Promise<void> {
...
export async function exportCommentsForAgent(
  ...
  return invoke<AgentExport>('export_comments_for_agent', { repoPath, revision });
}
